...
resource "aws_db_instance" "fiap-db-status" {
  allocated_storage      = 20 # Tamanho em GB
  storage_type           = "gp2"
  engine                 = "postgres"
  engine_version         = "12.16"
  instance_class         = "db.t2.micro"     # Tipo de instância
  identifier             = "fiap-db-status"      # Identificador da instância RDS
  name                   = "fiapStatus"            # Nome da base de dados dentro da instância
